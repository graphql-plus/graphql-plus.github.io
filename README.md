...
1) Run the `build-local.ps1` script which requires the following to be installed:
  a) A recent Dotnet SDK (8.0, 9.0 and 10.0 are tested)
  b) The Node Version Selector tool (nvs) to be installed (as node 22 is currently required by docfx)

or
2) Run the customised docfx container built out of the included Dockerfile with:
